...
SELECT 
    ...
    COALESCE(
    y.metric_array || repeat(null, cast(date_diff('day', date('2023-08-01'), t.date) as integer)), 
    ARRAY[t.metric_value]
    ) AS metric_array, -- Append today's metric value to the existing metric array
